...
When (/^I create test run in TestRail$/) do
  begin
	...
	name = "BigTed ActiveX #{versionId} - #{newtime}"
	desc = "#{description} - #{versionId}"
	caseids = $caseids
	createRun(projectId, suiteId, name, desc, caseids)
  rescue
    puts "I create test run in TestRail has Faield"
  end
end
...
def posttestrail(runId, caseId, statusId, versionId, elapsedseconds)
  ...
  files = "//zgbwcfs3005.jupiter.bbc.co.uk/QA/Jenkins/Jupiter/ACTIVEXresultupdatelog.txt"
  f = File.open(files,'a')
  f.write "#{testID} #{timeStamp}"
  f.close
	rescue
		puts "posttestrail has Failed"
	end
end
